...
#    max_this_nTrees = 0
    ...
    nTreeHalos = numpy.zeros(nTrees)
    output_Galaxy = numpy.zeros(nHalos)
    treeCounter = 0
    galaxyCounter = 0
    for ifile in range(firstfile,lastfile+1): 
        filename = folder+'/'+file_prefix+"_"+"%d"%(ifile)
...
#        nTreeHalos = numpy.append(nTreeHalos,addednTreeHalos)
        nTreeHalos[treeCounter] = addednTreeHalos
        this_addedGalaxy = numpy.fromfile(f,LGalaxyStruct.struct_dtype,this_nHalos)
        addedGalaxy = numpy.zeros(this_nHalos,dtype=filter_dtype)
        for prop in LGalaxyStruct.struct_dtype.names:
            if(filter_arr[prop] is True):
                addedGalaxy[prop] = this_addedGalaxy[prop]
#        output_Galaxy = numpy.append(output_Galaxy,addedGalaxy)
        output_Galaxy[galaxyCounter] = addedGalaxy       
        treeCounter=treeCounter+1
        GalaxyCounter=GalaxyCounter+nHalos*len(K=LGalaxyStruct.struct_dtype.names)
...
def read_snap(folder,file_prefix,firstfile,lastfile,props,template):
    ...
    filter_dtype = np.dtype(filter_list)
    #gals = np.array([],dtype=filter_dtype)
    #Find out how many galaxies there are
    nTrees = 0
    nHalos = 0
    for ifile in range(firstfile,lastfile+1):
        filename = folder+'/'+file_prefix+"_"+"%d"%(ifile)
 ...
 #       nTreeHalos = np.append(nTreeHalos,addednTreeHalos)
        nTreeHalos[nTrees:nTrees+this_nTrees] = addednTrees
