...
if eddr0>1e-10 and agnlf_type!=0: raise ValueError('eddr0>1e-10 and gnlf_type!=0')
...
if agnlf_type==0:
    if eddr0 > 1e-10:
        ...
        pngfile='figs/agnlf_agnfeedback_total_e'+str(eddr0)+'_'+model+'.png'
    else:
        Title='AGN luminosity from total BH accretion rate'
        pngfile='figs/agnlf_agnfeedback_total_'+model+'.png'
elif agnlf_type==1:
    Title='AGN luminosity from Quasar accretion rate'
    pngfile='figs/agnlf_agnfeedback_qar_'+model+'.png'
elif agnlf_type==2:
    Title='AGN luminosity from Radio accretion rate'
    pngfile='figs/agnlf_agnfeedback_rar_'+model+'.png'
...
for redshift in redshifts:
    ...
    if eddr0 > 1e-10:
        ...
        print("AGNLF*eddr/EddR:"+str(AGNLF*eddr/EddR) )
        print("y: "+str(y))
    else:
    ...
    print("----------------------------------------")
    print(y)
    plt.plot(x,10**y,label='z= '+str(redshift))

plt.legend([str(redshift) for redshift in redshifts],loc=3)
plt.text(12.5,2e-3,'model = '+model,ha='right')
if eddr0>1e-10: 
    if eddr_power > 0:
        plt.text(12.5,4e-3,'Eddington ratio = '+str(eddr0)+'(1+z)^'+str(eddr_power),ha='right')
    else:
        plt.text(12.5,4e-3,'Eddington ratio = '+str(eddr0),ha='right')
...
for redshift in redshifts:
    if obs=='LRS15':
        ...
        plt.text(12.5,1e-3,'obs = LRS15',ha='right')
    if obs=='HRH06':
        HRH06.plot(xmin,xmax,
                   z=redshift,hubble_units=hubble_units,linestyle=':')
        plt.text(12.5,1e-3,'obs = HRH06',ha='right')
