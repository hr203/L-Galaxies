...
#ifdef INDIVIDUAL_ELEMENTS
    Yield = elements_add(elements_init(),Gal[q].FarEjectedMass_elements,fraction);
#endif
  } 
#endif
else {
...
#ifdef INDIVIDUAL_ELEMENTS
    Gal[p].FarEjectedMass_elements = elements_add(Gal[p].FarEjectedMass_elements,Yield,1.);
#endif
  }
#endif
else {
...
#ifdef INDIVIDUAL_ELEMENTS
    Gal[q].FarEjectedMass_elements = elements_add(Gal[q].FarEjectedMass_elements,Yield,-1.);
#endif
  }
#endif
...
void mass_checks(char string[], int igal) {
  ...
  if(Gal[igal].FarEjectedMass < 0.0) {
    if (Gal[igal].FarEjectedMass > -1e-7)
      ...
      printf("\n*** Mass check error, called from: %s, FarEjectedMass < 0. ***\n",string);
      printf("                FarEjectedMass[%d] = %g\n",igal,Gal[igal].FarEjectedMass);
      terminate("");
    }
  }

  if(metals_total(Gal[igal].MetalsFarEjectedMass) < 0.0) {
    if (metals_total(Gal[igal].MetalsFarEjectedMass) > -1e-7)
      Gal[igal].MetalsFarEjectedMass = metals_init();
    else {
      printf("\n*** Mass check error, called from: %s, MetalsFarEjectedMass < 0. ***\n",string);
      printf("                MetalsFarEjectedMass[%d] = %g\n",igal,metals_total(Gal[igal].MetalsFarEjectedMass));
      terminate("");
    }
  }

  if(metals_total(Gal[igal].MetalsFarEjectedMass) > Gal[igal].FarEjectedMass) {
    if (metals_total(Gal[igal].MetalsFarEjectedMass) < 1e-7)
      Gal[igal].MetalsFarEjectedMass = metals_add(metals_init(),Gal[igal].MetalsFarEjectedMass,
                                           Gal[igal].FarEjectedMass/metals_total(Gal[igal].MetalsFarEjectedMass));
    else {
      printf("\n*** Mass check error, called from: %s, MetalsFarEjectedMass > FarEjectedMass ***\n",string);
      printf("          MetalsFarEjectedMass[%d] = %g\n",igal,metals_total(Gal[igal].MetalsFarEjectedMass));
      printf("                FarEjectedMass[%d] = %g\n",igal,Gal[igal].FarEjectedMass);
      terminate("");
    }
