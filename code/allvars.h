...
#ifdef AGNFB
  float FarEjectedMass; //10^10/h Msun - Mass in outer resevoir
#endif
  float BlackHoleMass; // 10^10/h Msun - Mass in black hole
...
#ifdef AGNFB
  struct metals MetalsFarEjectedMass; //10^10/h Msun -  Mass in metals in far ejected gas 
#endif
...
#ifdef AGNFB
  float MetalsFarEjectedMass; //10^10h/ Msun -  Mass in metals in the far ejected gas
#endif
...
#ifdef AGNFB
  struct elements FarEjectedMass_elements;
#endif
#endif //INDIVIDUAL_ELEMENTS
};
...
#ifdef AGNFB
  struct metals MetalsFarEjectedMass;
#endif 
#ifdef METALS_SELF
