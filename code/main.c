  ...
  for (nstep = 0; nstep < STEPS; nstep++)
    {
	  ...
	  if (BlackHoleGrowth == 1)
	  {
		  for (p = 0; p < ngal; p++)
		  {
			...
			if (AGNaccreted > 0.)
                        {
                                 if (AGNaccreted <= Gal[p].BlackHoleGas)
				 {
				...
				{
					printf("%s \n","Trying to accrete more gas than there is in the disk");
                                        Gal[p].BlackHoleMass += Gal[p].BlackHoleGas;
                                        Gal[p].BlackHoleGas -= Gal[p].BlackHoleGas;
				}
...
#ifdef AGNFB
                                AGN_feedback(p, Gal[p].CentralGal, AGNaccreted, Gal[p].Vvir, "ColdGas");
#endif
